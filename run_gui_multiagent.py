...
def main():
  ...
  parser.add_argument('--window', type=int, nargs=2, default=(1000, 1000))
  ...
  while running:
    ...
    actions = {f'{i}': env.action_names.index('noop') for i in range(env.n_players)}
    actions[f'{env.player_id}'] = env.action_names.index(action)
    #print(env._players[env.player_id].pos)
